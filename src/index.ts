...
// Log MCP initialize lifecycle
(server as any).server.oninitialized = () => {
    console.error('MCP initialized: client connected and ready.');
};
// Trace wrapper for initialize request/response
try {
    const low = (server as any).server;
    const initMethod = InitializeRequestSchema.shape.method.value as string;
    const orig = low._requestHandlers?.get(initMethod);
    if (orig) {
        low._requestHandlers.set(initMethod, (request: any, extra: any) => {
            try {
                console.error('[trace] initialize request:', JSON.stringify(request));
            } catch {}
            const p = Promise.resolve(orig(request, extra));
            return p.then((result: any) => {
                try {
                    console.error('[trace] initialize response:', JSON.stringify(result));
                } catch {}
                return result;
            }).catch((err: any) => {
                console.error('[trace] initialize error:', err);
                throw err;
            });
        });
    } else {
        console.error('[trace] initialize handler not found on server._requestHandlers');
    }
    // Also trace tool list and tool call
    const listMethod = ListToolsRequestSchema.shape.method.value as string;
    const callMethod = CallToolRequestSchema.shape.method.value as string;
    const origList = low._requestHandlers?.get(listMethod);
    const origCall = low._requestHandlers?.get(callMethod);
    if (origList) {
        low._requestHandlers.set(listMethod, (request: any, extra: any) => {
            console.error('[trace] tools/list request');
            return Promise.resolve(origList(request, extra)).then((result: any) => {
                try { console.error('[trace] tools/list response:', JSON.stringify(result)); } catch {}
                return result;
            }).catch((err: any) => { console.error('[trace] tools/list error:', err); throw err; });
        });
    }
    if (origCall) {
        low._requestHandlers.set(callMethod, (request: any, extra: any) => {
            try { console.error('[trace] tools/call request:', JSON.stringify(request)); } catch {}
            return Promise.resolve(origCall(request, extra)).then((result: any) => {
                try { console.error('[trace] tools/call response:', JSON.stringify(result)); } catch {}
                return result;
            }).catch((err: any) => { console.error('[trace] tools/call error:', err); throw err; });
        });
    } else {
        console.error('[trace] tools/call handler not found at startup (will be registered on first tool)');
    }
    // Monkey-patch setRequestHandler to auto-wrap future registrations
    const origSet = low.setRequestHandler.bind(low);
    low.setRequestHandler = (schema: any, handler: any) => {
        const methodName = schema?.shape?.method?.value as string;
        let wrapped = handler;
        if (methodName === (CallToolRequestSchema.shape.method.value as string)) {
            wrapped = (request: any, extra: any) => {
                try { console.error('[trace] tools/call request (late):', JSON.stringify(request)); } catch {}
                return Promise.resolve(handler(request, extra)).then((result: any) => {
                    try { console.error('[trace] tools/call response (late):', JSON.stringify(result)); } catch {}
                    return result;
                }).catch((err: any) => { console.error('[trace] tools/call error (late):', err); throw err; });
            };
        }
        if (methodName === (ListToolsRequestSchema.shape.method.value as string)) {
            wrapped = (request: any, extra: any) => {
                console.error('[trace] tools/list request (late)');
                return Promise.resolve(handler(request, extra)).then((result: any) => {
                    try { console.error('[trace] tools/list response (late):', JSON.stringify(result)); } catch {}
                    return result;
                }).catch((err: any) => { console.error('[trace] tools/list error (late):', err); throw err; });
            };
        }
        return origSet(schema, wrapped);
    };
} catch (e) {
    console.error('[trace] failed to wrap initialize handler:', e);
}
